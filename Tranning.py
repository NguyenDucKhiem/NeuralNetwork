...
train_data_week, test_data_week = train_test_split(matrix[1:2].T, test_size=test_size, random_state=random_state)
train_data_year, test_data_year = train_test_split(matrix[0:1].T, test_size=test_size, random_state=random_state)
train_data_patient, test_data_patient = train_test_split(matrix[2:3].T, test_size=test_size, random_state=random_state)
...
# Input
week_inputs = Input(shape=(1,), name='week')
year_inputs = Input(shape=(1,), name='year')
# Hiden layer
layer = Dense(4, input_shape=(1,), activation='tanh', name='Week_Layer')(week_inputs)
layer = Dense(6, activation='tanh', name='Week_Layer_2')(layer)
layer = Dropout(0.5)(layer)

layer2 = Dense(2, input_shape=(1,), activation='tanh', name='Year_Layer')(year_inputs)
...
#   Fit/train the model
history = model.fit([train_data_week_norm, train_data_year_norm], train_data_patient, nb_epoch=100, verbose=2, callbacks=my_callbacks)

# Note: fit cost values will be different because we did not use NN in original.
score = model.evaluate([test_data_week_norm, test_data_year_norm], test_data_patient)
